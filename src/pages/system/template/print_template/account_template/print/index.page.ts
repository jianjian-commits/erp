...
import { LocalStorage, setTitle } from '@gm-common/tool'
...
import { CustomerUser } from 'gm_api/src/enterprise'
import {
...
const Print = () => {
  ...
  const { template_id, query, showRise, childTypeValue } = location.query
  const { fetchListCustomerUser } = store
  ...
  function getSinglePrintTemplate(templateId: string = template_id) {
    return GetPrintingTemplate({ printing_template_id: templateId }).then(
      (json) => {
  ...
  > {
    ...
    return Promise.all(reqList)
      .then((res: any[]) => {
        ...
        const orders = _.map(newResponseList, (json) => {
  ...
  async function getDeliveryConfigList(
    ...
    try {
      // 后台返回数据台乱。。 整理后台返回的数据
      const list = handleOrderPrinterData(
        printData?.dataList as ListOrderWithRelationResponse,
      )
      if (template_id !== 'company_config') {
        // 正常模板配置
        ...
        if (to_print_task === 'true') {
          ...
          // driverTaskPrintList = handleDriverPrintList(
          //   driverTaskList,
          //   driverTaskConfig,
          //   template,
          // )
          // printOrderList = printOrderList.concat(driverTaskPrintList)
        }
        ...
        // 抽取orders里面关联的customer_id， 去重减少相同请求
        const customer_ids = Array.from(
          new Set(
            _.map(orders, (order) => {
              // 账户和商户打印用的customer_id 取值不同
              return relation_info?.customers?.[order?.customer_id_l1]
                ?.customer_id
            }),
          ),
        )
        ...
        try {
          ...
          customerUsers = await fetchListCustomerUser(customer_ids)
        ...
        const printOrderList = _.reduce(
          ...
          async (all, customerId) => {
            ...
            // 账户和商户打印用的customer_id 取值不同
            let order = list.find((item) => item.customer_id_l1 === customerId)

            const customerUserInfo = customerUsers.find(
              (v) => v.customer_id === customerId,
            )
            order = Object.assign(
              {
                username: customerUserInfo?.username, // 公司登录账号信息
              },
              order,
            )
