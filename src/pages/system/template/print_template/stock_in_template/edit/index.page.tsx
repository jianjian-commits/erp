...
import { EditorStockIn } from 'gm-x-printer'
import { Tip, LoadingChunk } from '@gm-pc/react'
import defaultConfig from '../config/template_config/stock_in_config'
import mockData from '../config/mock_data/stock_in_data.json'
import ShelfData from '../config/mock_data/shelf_data.json'
import { setTitle } from '@gm-common/tool'
import stockInDataKey from '../config/data_to_key'
...
const TemEditor = () => {
  ...
  if (loading) {
    ...
    return (
      <EditorStockIn
        config={content}
        mockData={stockInDataKey(mockData, { shelfList: ShelfData.shelves })}
        onSave={handleSave}
        showEditor
        addFields={addFields}
        uploadQiniuImage={(file: File) => {
          return uploadQiniuImage(
            FileType.FILE_TYPE_INVENTORY_IN_TEMPLATE,
            file,
          )
        }}
      />
