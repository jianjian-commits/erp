...
import mockData from '../config/mock_data/purchase_bill_data.json'
...
const TemEditor = () => {
  ...
  if (loading) {
    ...
    const { purchase_sheet, ...rest } = mockData
    return (
      <EditorPurchase
        config={content}
        mockData={purchaseBill(purchase_sheet, rest)}
        onSave={handleSave}
        showEditor
        addFields={addFields}
        isPurchase
        uploadQiniuImage={(file: File) => {
          return uploadQiniuImage(
            FileType.FILE_TYPE_PURCHASE_BILL_TEMPLATE,
            file,
          )
        }}
      />
