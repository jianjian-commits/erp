...
import { EditorSettle } from 'gm-x-printer'
import { Tip, LoadingChunk } from '@gm-pc/react'
import defaultConfig from '../config/template_config/supplier_settle_config'
import mockData from '../config/mock_data/supplier_settle_data.json'

import { setTitle } from '@gm-common/tool'
import settleDataToKey from '../config/data_to_key'
...
const TemEditor = () => {
  ...
  if (loading) {
    ...
    return (
      <EditorSettle
        config={content}
        mockData={settleDataToKey(mockData)}
        onSave={handleSave}
        showEditor
        addFields={addFields}
        uploadQiniuImage={(file: File) => {
          return uploadQiniuImage(
            FileType.FILE_TYPE_FINANCE_SETTLE_SHEET_TEMPLATE,
            file,
          )
        }}
      />
