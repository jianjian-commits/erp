...
const Supplier = () => {
  ...
  const handleDelete = (record: PurchaseRule) => {
    ...
    const { supplier_id, purchaser_id, level_field_id, purchase_rule_id } =
      record
    const name = supplier_map?.[supplier_id!]?.name || '-'

    confirm({
      title: t('删除供应商'),
      icon: <ExclamationCircleOutlined />,
      content: t(`确定删除该规则中的${name}的供应商吗`),
      cancelText: t('取消'),
      okText: t('确定'),
      onOk() {
        if (purchaser_id === '0' && level_field_id === '0') {
  ...
  return (
    <>
      <Table
        style={{ height: '100%' }}
        columns={columns}
        dataSource={merchandiseTableListSupplier.slice()}
        pagination={false}
        loading={loading}
      />
      {visible && (
