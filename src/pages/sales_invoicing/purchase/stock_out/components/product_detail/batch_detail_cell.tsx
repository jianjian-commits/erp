...
const log = debug(
  'sales_invoicing/purchase/stock_out/components/product_detail/batch_detail_cell.tsx',
)
interface Props {
  index: number
  data: PDetail
...
const verifyBatch = (
  ...
  _.each(data, (item) => {
    if (!isValid(item.sku_base_quantity) || !isValid(item.ssu_quantity)) {
      ...
      log(
        'item.sku_base_quantity, item.ssu_quantity',
        item.sku_base_quantity,
        item.ssu_quantity,
      )
  ...
  if (isBatchValid && flag && totalBaseQuantity !== total) {
    ...
    // Tip.tip(t('所选批次出库数（基本单位）需要等于待出库数'))
    Dialog.render({
      title: '提示',
      buttons: [
        {
  ...
  const handleEnsure = (batchSelected: BatchDetail[]) => {
    ...
    let base_quantity_ =
      ...
      +Big(
        batchSelected
          ...
          .reduce((a, c) => a + c, 0),
      ).div(second_base_unit_ratio || 1)

    const skuBaseCount = +Big(base_quantity_)
      // .times(second_base_unit_ratio || 1)
      .times(1)
      .toFixed(globalStore.dpSalesInvoicing)
    ...
    const fn = (free = false) => {
      ...
      store.changeProductDetailsItem(index, {
        ...
        // amount_show: amount,
        // ssu_base_price,
        // ssu_base_price_show: ssu_base_price,
        base_price,
        no_tax_base_price: base_price,
        no_tax_amount: amount,
      })

      // const { ssu_quantity, ssu_quantity_show } = getLinkCalculate({
      //   data,
      //   currentField: 'ssu_base_quantity',
      //   currentValue: ssu_base_quantity_,
      // })

      // TODO： 暂时不需要进行second_base_unit_ratio换算，后续需要补充
      const secondInputValue = Big(Number(base_quantity_) || 0).times(
        // Number(second_base_unit_ratio || 1),
        Number(1),
      )
    ...
    if (verifyBatch(batchSelected, skuBaseCount, !!base_quantity_, fn)) {
      fn()
      RightSideModal.hide()
    }
  }
