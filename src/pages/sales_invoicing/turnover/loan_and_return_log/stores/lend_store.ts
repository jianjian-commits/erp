...
const initFilter: FtSheet = {
  ...
  customer_q: '',
  sku_q: '',
  stock_sheet_type: RECEIPT_TYPE.turnoverLoan,
...
class Store {
  ...
  getSearchData() {
    const { begin_time, end_time, select_type, warehouse_id, ...other } =
      this.filter
  ...
  createSheet() {
    return CreateStockSheet(getSearchCustomer(this.sheetInfo, 'turnoverLoan'))
  }
  ...
  updateSheet(index: number, status?: StatusType) {
    ...
    const { quantity, ...other } = this.list[index]
