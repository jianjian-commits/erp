...
const columns: Column[] = [
  ...
  },
  {
    ...
    accessor: 'target_id',
    Cell: (cellProps) => {
      const { target_customer_id, customerInfo } = cellProps.original

      return target_customer_id === '0'
        ? '-'
        : getUnNillText(customerInfo?.customized_code)
    },
  },
  {
    Header: t('客户名称'),
    ...
    Cell: (cellProps) => {
      const { target_name, customerInfo } = cellProps.original
      return target_name || customerInfo?.name
    },
  },
  // {
  //   Header: t('规格'),
