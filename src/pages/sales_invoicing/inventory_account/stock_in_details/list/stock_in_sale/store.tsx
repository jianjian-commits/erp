...
const initFilter: FilterType = {
  ...
  q: '',
  target_id: '0',
  operate_type: OPERATE_TYPE.refundIn, // 销售退货入库
...
class Store {
  ...
  _handleExport = () => {
    return ExportStockLog({
      ...
      task_type: EXPORT_TASK_TYPE.saleRefundIn,
    })
  }
  ...
  getSearchData() {
    const { begin_time, end_time, category_ids, target_id, ...another } =
      this.filter

    return {
      begin_time: '' + moment(begin_time).format('x'),
      end_time: '' + moment(end_time).format('x'),
      target_customer_ids: +target_id! ? [target_id] : [],
      ...another,
    }
  }
