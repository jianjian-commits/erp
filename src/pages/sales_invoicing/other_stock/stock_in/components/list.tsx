...
const ListRight = observer(() => {
  const handleCreate = () => {
    history.push('/sales_invoicing/other_stock/stock_in/create')
  }

  return (
    <PermissionJudge
      permission={Permission.PERMISSION_INVENTORY_CREATE_OTHER_IN}
    >
...
const TabList: FC<InComing> = observer((props) => {
  ...
  const columns: Column[] = [
    ...
    },
    {
      ...
      Cell: (cellProps) => {
        const { stock_sheet_id, stock_sheet_serial_no, sheet_status, status } =
          cellProps.original
        return (
          <StockSheetLink
            url='/sales_invoicing/other_stock/stock_in'
              ...
              <OperationDelete
                disabled={
                  !globalStore.hasPermission(
                    Permission.PERMISSION_INVENTORY_DELETE_OTHER_IN,
                  )
                }
                title={t('确认删除')}
                onClick={() => {
    ...
    >
      <Table
        ...
        batchActions={_.without(
          [
            globalStore.hasPermission(
              Permission.PERMISSION_INVENTORY_PRINT_OTHER_IN,
            ) && {
              children: (
