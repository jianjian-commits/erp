...
import { Table, TableXUtil, BatchActionDefault, Column } from '@gm-pc/table-x'
...
const ListRight = observer(() => {
  const handleCreate = () => {
    history.push('/sales_invoicing/produce/refund_stock_in/create')
  }

  return (
    <PermissionJudge
      permission={Permission.PERMISSION_INVENTORY_CREATE_MATERIAL_IN_251}
    >
    ...
    return [
      ...
      },
      {
        ...
        Cell: (cellProps) => {
          ...
          return (
            <StockSheetLink
              url='/sales_invoicing/produce/refund_stock_in'
      ...
      },
      {
        Header: t('领用部门'),
        Cell: (cellProps) => {
          const { processor_name } = cellProps.original
          return processor_name || t('未指定')
        },
        minWidth: 100,
      },
      {
      ...
      },
      {
        ...
        Cell: ({ original: { remark } }) => {
          return isValid(remark) ? <TableTextOverflow text={remark} /> : '-'
        },
      },
      {
        Header: t('建单人'),
