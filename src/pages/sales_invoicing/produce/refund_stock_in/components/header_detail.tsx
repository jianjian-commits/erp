    ...
    if (stock_sheet_id) {
      ...
      store
        ...
        .then((json) => {
          ...
          history.push(
            `/sales_invoicing/produce/refund_stock_in/detail?sheet_id=${
  ...
  return (
    <>
      {currentActionList.includes('submitted') && (
        <PermissionJudge
          permission={Permission.PERMISSION_INVENTORY_SUBMIT_MATERIAL_IN_253}
        >
          <Button
            type='primary'
      ...
      {currentActionList.includes('approved') && (
        <PermissionJudge
          permission={Permission.PERMISSION_INVENTORY_APPROVE_MATERIAL_IN_254}
        >
          <Button
            type='primary'
            className='gm-margin-right-5'
      ...
      {sheet_status === -1 && currentActionList.includes('toBeSubmitted') && (
        <PermissionJudge
          permission={Permission.PERMISSION_INVENTORY_UPDATE_MATERIAL_IN_252}
        >
          <div className='gm-gap-10' />
          <Button
            plain
          ...
          data={_.without(
            [
              ...
              globalStore.hasPermission(
                Permission.PERMISSION_INVENTORY_NOT_APPROVE_MATERIAL_IN_255,
              ) && {
                text: t('驳回'),
              ...
              globalStore.hasPermission(
                Permission.PERMISSION_INVENTORY_CANCEL_APPROVE_MATERIAL_IN_256,
              ) && {
                text: t('反审核'),
              ...
              // },
              globalStore.hasPermission(
                Permission.PERMISSION_INVENTORY_DELETE_MATERIAL_IN_257,
              ) && {
                text: t('删除单据'),
...
const HeaderDetail: FC<HeaderDetailProps> = observer((props) => {
  ...
  const renderHeaderInfo = () => {
    ...
    const { listMaterialOrder } = store

    return [
      {
        label: t('退料入库单号'),
        item: (
          <div style={{ width: '380px' }}>{stock_sheet_serial_no || '-'}</div>
        ),
      },
      {
        label: t('关联领料单'),
          ...
          type === 'add' ? (
            <Select_MaterialOrder
              ...
              onChange={(value: string) => {
                const serial_no = listMaterialOrder.find(
                  (item) => item?.material_order_id === value,
                )?.serial_no
                console.log('value', value, serial_no)
                store.changeReceiptDetail('material_order_id', value)
              ...
              getResponseData={store.saveListMaterialOrder}
        ...
        item:
          type === 'add' ? (
            <LevelSelect
              ...
              placeholder={t('请选择退料部门')}
              data={store.processors?.slice()}
              onSelect={(value) => {
                store.changeReceiptAllDetail(
                  Object.assign(store.receiptDetail, {
                    processor_ids: value,
                    processor_name: getProcessName(store.processorsList, value),
                  }),
                )
              }}
            />
