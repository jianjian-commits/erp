...
import { getFormatTimeForTable, parseSsu } from '@/common/util'
...
// const getSsu = (originalRow: Batch) => {
      ...
      const columns_: Column<Batch>[] = [
        {
          Header: t('规格'),
          accessor: () => {
            return center(ssu_name)
          },
        },
        { Header: t('批次号'), accessor: 'batch_serial_no' },
        {
          Header: t('供应商'),
          accessor: (originalRow: Batch) => {
            if (
        ...
        {
          Header: t('账面库存（计量单位）'),
          accessor: (originalRow: Batch, index: number) => {
            const unit_id = originalRow.stock?.sku_unit?.unit_id
        ...
        //     const { ssu_unit_name } = parseSsu(ssu)
        //     const quantity = originalRow.stock?.sku_unit?.quantity
        //     if (!quantity || !ssu_unit_name) return omissionMark

        //     const result = +Big(quantity).toFixed(4) + separator + ssu_unit_name

        //     set(this.base_unit_stock.package_unit, index, result)
        //     set(
        //       this.base_unit_stock.package_unit_id,
        //       index,
        //       originalRow.sku_unit_id,
        //     )
      ...
      if (!this.canEdit) {
        ...
        return [
          ...
          },
          {
            ...
            Cell: (cellProps) => {
              const { index, original } = cellProps
              const package_unit_stock = get(
                this.base_unit_stock.package_unit,
                index,
              ) as string

              if (!package_unit_stock) return omissionMark

              const [counts, units] = package_unit_stock.split(separator)
              return (
                <Flex column style={{ width: '120px' }}>
                  <Observer>
                    {() => (
                      <Flex justifyCenter alignCenter>
                        <InputNumber
                          ...
                          onChange={action((value) => {
                            if (value === null) {
                              set(this.input, index, value)
                              set(this.input2, index, value)
                              return void 0
                            }
                            ...
                            if (value! >= +counts) {
                              ...
                              set(
                                this.input,
                                index,
                                +Big(value!).times(getRate()).toFixed(4),
                              )
                            }
                          })}
                          style={{ width: TABLE_X.WIDTH_NUMBER }}
              ...
              Dialog.render({
                ...
                buttons: [
                  ...
                  },
                  {
                    ...
                    text: '确定',
                    onClick: action(() => {
                      const details: Array<StockSheet_Detail> = ls.input
                        .map<StockSheet_Detail | boolean>((it, idx) => {
                          ...
                          const package_count = ls.input2[idx] as number
                          const sku_unit_id_ =
                          ...
                          const input2 =
                            +base_unit_id >= 200000
                              ...
                              : {
                                  price: '0',
                                  quantity: package_count.toString(),
                        ...
                        .filter(Boolean) as Array<StockSheet_Detail>
