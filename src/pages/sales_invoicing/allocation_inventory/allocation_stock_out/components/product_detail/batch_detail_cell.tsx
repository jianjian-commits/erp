...
const BatchDetailCell: FC<Props> = observer(({ index, data, warehouseId }) => {
  ...
  const handleEnsure = (batchSelected: BatchDetail[]) => {
    ...
    const fn = (free = false) => {
      ...
      // 均价
      const average_price = batchSelected
        .map((it) => {
          return +it.batch_average_price! * +it.sku_base_quantity! ?? 0
        })
        .reduce((a, c) => a + c, 0)
      ...
      store.changeProductDetailsItem(index, {
        ...
        tax_input_price: +Big(average_price).div(base_quantity_ || 1),
  ...
  const handleBatch = () => {
    ...
    const skuBaseCount = +Big(base_quantity ?? 0).toFixed(
      globalStore.dpSalesInvoicing,
    )
