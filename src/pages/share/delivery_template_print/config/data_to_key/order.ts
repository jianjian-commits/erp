...
import {
  price,
  PayState,
} from '@/pages/system/template/print_template/delivery_template/util'
...
function generateOrderData(list: any) {
  return _.map(list, (v, index) => {
    return {
      ...
      不含税商品销售额: price(`${+v?.sale_price! / (+v?.ssu.tax! / 100 + 1)}`),
 ...
 */
function formatOrder(data: any, appId?: string) {
  // 处理数据
  // 商品列表
